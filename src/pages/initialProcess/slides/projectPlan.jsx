import React, { useEffect, useState } from "react";
import { LinkOutlined } from "@ant-design/icons";
import { Button, Form, Input, Result, Space, Spin, Upload } from "antd";
import Title from "antd/es/typography/Title";
import Paragraph from "antd/es/typography/Paragraph";
import apiServices from "../../../services/exportService";
import { apiEndpoints } from "../../../utils/apiEndPoints";
import {
  readSectionId,
  readSelectedProjectId,
  readStudentId,
  readUserId,
} from "../../../services/localServices";
import appImages from "../../../data/images";
import "../style.css";
import { UploadOutlined } from "@ant-design/icons";
const ProjectPlanSlide = ({ setCurrentTab, currentTab }) => {
  const [form] = Form.useForm();
  const [planStatus, setPlanStatus] = useState("loading"); //laoding, none, pending, done
  const [FetchedData, setFetchedData] = useState("");
  const [selectedFile, setSelectedFile] = useState([]);
  const [ReSubmit, setReSubmit] = useState(false);
  useEffect(() => {
    onInitialize();
  }, []);

  const props = {
    onRemove: (file) => {
      const index = selectedFile.indexOf(file);
      const newFileList = selectedFile.slice();
      newFileList.splice(index, 1);
      setSelectedFile(newFileList);
    },
    beforeUpload: (file) => {
      setSelectedFile([file]);
      return false;
    },
    selectedFile,
  };

  //Basic Functions

  //main Action Functions
  const onInitialize = () => {
    apiServices
      .get(
        `${
          apiEndpoints.home.initialProcess.projectPlanning.get
        }${readSelectedProjectId()}/`
      )
      .then((res) => {
        if (res?.data.length === 0) {
          setPlanStatus("none");
        } else {
          if (res.data[0].status) {
            if (res.data[0].status === "Accepted") {
              setCurrentTab(1);
            }
            setPlanStatus(res.data[0].status);
            setFetchedData(res.data[0]);
          } else {
            setPlanStatus("none");
          }
        }
      })
      .catch((e) => console.log(e))
      .finally(() => {});
  };

  const handleSubmitPlan = (values) => {
    const formData = new FormData();
    let PostURL;
    if (ReSubmit) {
      PostURL = `${apiEndpoints.home.initialProcess.projectPlanning.update}${FetchedData.ppid}/`;
      formData.append("upload_projectplan", selectedFile[0]);
      formData.append("description", values.description);
    } else {
      PostURL = `${apiEndpoints.home.initialProcess.projectPlanning.create}`;
      formData.append("upload_projectplan", selectedFile[0]);
      formData.append("description", values.description);
      formData.append("User_Id", readUserId());
      formData.append("Project_ID", readSelectedProjectId());
      formData.append("stud_id", readStudentId());
      formData.append("Sec_ID", readSectionId());
    }
    apiServices
      .post(PostURL, formData)
      .then((res) => {
        setPlanStatus("UnderReview");
        console.log(res);
      })
      .catch((e) => console.log(e))
      .finally(() => {});
  };

  return (
    <>
      {planStatus === "loading" ? (
        <Spin />
      ) : planStatus === "none" ? (
        <UploadProjectPlanSection />
      ) : planStatus === "UnderReview" ? (
        <ProjectPlanUnderReview />
      ) : planStatus === "Accepted" ? (
        <ProjectPlanApproved />
      ) : planStatus === "Rejected" ? (
        <ProjectPlanRejected />
      ) : null}
    </>
  );

  function UploadProjectPlanSection() {
    return (
      <>
        {" "}
        <div className="flex flex-col items-center">
          <Space direction="vertical" className="items-center">
            <Title level={3}>Upload the project Plans</Title>

            <Paragraph>
              If you are using{" "}
              <a
                href="https://www.bitrix24.in/"
                target="_blank"
                style={{ textDecoration: "none" }}
              >
                Bitrix24{" "}
              </a>
              or similar websites for your project planning then please attach
              the url here.
            </Paragraph>

            <Form onFinish={handleSubmitPlan} form={form} layout="vertical">
              {/* <Upload
                style={{ width: "1000px" }}
                listType="picture-card"
                fileList={selectedFile}
                {...props}
              >
                {selectedFile.length < 1 && "+ Upload"}
              </Upload> */}
              <Upload
                {...props}
                listType="picture"
                className="upload-list-inline"
                defaultFileList={[...selectedFile]}
                // onPreview={onPreview}
                maxCount={1}
              >
                {selectedFile.length === 0 && (
                  <Button
                    icon={<UploadOutlined />}
                    style={{ width: "350px", height: "60px" }}
                  >
                    Upload
                  </Button>
                )}
              </Upload>
              <Form.Item label={"Description"} name="description">
                <Input
                  name="description"
                  type="Description"
                  size="large"
                  placeholder="Enter your project plan description if any"
                  prefix={<LinkOutlined />}
                />
              </Form.Item>

              <Button
                className="w-full"
                htmlType="submit"
                size="large"
                type="primary"
              >
                Request for Approval
              </Button>
            </Form>
          </Space>
        </div>
      </>
    );
  }

  function ProjectPlanUnderReview() {
    return (
      <>
        <Result
          icon={
            <img
              className="h-20"
              src={appImages.pageWise.initialProcess.underreview}
            />
          }
          title="Your project plan is under review!"
          extra={<></>}
        />
      </>
    );
  }

  function ProjectPlanApproved() {
    return (
      <>
        <Result
          status="success"
          title="Great! Your project plan is approved."
          subTitle=""
          extra={[
            <Button
              onClick={() => setCurrentTab(1)}
              type="primary"
              key="console"
            >
              Next
            </Button>,
          ]}
        />
      </>
    );
  }

  function ProjectPlanRejected() {
    return (
      <>
        <Result
          status="error"
          title="Project plan Rejected"
          subTitle="Please revisit your project plan and do the needful."
          extra={[
            <Button
              type="primary"
              key="console"
              onClick={() => {
                setPlanStatus("none");
                setReSubmit(true);
              }}
            >
              Try Again
            </Button>,
            <Button key="buy">Contact Teacher</Button>,
          ]}
        ></Result>
      </>
    );
  }
};
export default ProjectPlanSlide;
